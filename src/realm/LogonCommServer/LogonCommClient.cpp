...
void LogonCommClientSocket::HandleRequestAccountMapping(WorldPacket & recvData)
{
    ...
    result = sCharSQL->Query("SELECT acct FROM characters");
...
void LogonCommClientSocket::HandleModifyDatabaseResult(WorldPacket& recvData)
{
    ...
    case Method_Account_Change_PW:
    {
        ...
        Session* pSession = sClientMgr.FindSessionByName(account_string);
        if (pSession == nullptr)
        {
...
void LogonCommClientSocket::HandleResultCheckAccount(WorldPacket& recvData)
{
    ...
    auto session_name = sClientMgr.FindSessionByName(request_string);
    ...
    case 3:     // Everything is okay
    {
        ...
        sCharSQL->Execute("REPLACE INTO account_forced_permissions (`login`, `permissions`) VALUES ('%s', '%s')", account_string, gmlevel_string);
        ...
        auto updated_account_session = sClientMgr.FindSessionByName(account_string);
